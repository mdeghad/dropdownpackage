...
class SearchableDropdown<T> extends StatefulWidget {
  ...
  SearchableDropdown._({
    ...
    this.menuBackgroundColor,
        ...
        assert(!multipleSelection || doneButton != null || closeButton != null),
  ...
  @override
  SearchableDropdownState<T> createState() => new SearchableDropdownState();
}
...
class SearchableDropdownState<T> extends State<SearchableDropdown<T>> {
  ...
  dynamic get selectedResult {
    return (widget.multipleSelection
        ? selectedItems
        : (selectedItems?.isNotEmpty ?? false) && selectedItems.first < widget.items.length
  ...
  void didUpdateWidget(SearchableDropdown oldWidget) {
    ...
    _updateSelectedIndex();
    // print('didUpdateWidget ${widget?.value} $selectedItems');
  }
  ...
  Widget build(BuildContext context) {
    ...
    // print('before exception -- value ${widget.value} for length ${widget.items.length} $selectedItems');
...
class _DropdownDialogState<T> extends State<DropdownDialog> {
  ...
  new TextStyle(fontSize: 16, fontWeight: FontWeight.normal);
  ...
  dynamic get selectedResult {
    return (widget.multipleSelection
        ...
        : (widget.selectedItems?.isNotEmpty ?? false) && (widget.items?.isNotEmpty ?? false)
    ...
    return AnimatedContainer(
      ...
      child: new Card(
        ...
        margin: EdgeInsets.symmetric(
            vertical: widget.dialogBox ? 95 : 5,
            horizontal: widget.dialogBox ? 55 : 4),
        child: new Container(
  ...
  Widget searchBar() {
    double width = MediaQuery
        ...
        .size
        .width;
    width>600?FocusScope.of(context).unfocus():FocusScope.of(context).requestFocus();
